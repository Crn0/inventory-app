...
const sequence_detail = asyncHandler(async (req, res, next) => {
    ...
    const sequence = await Sequence.findById(id)
        .populate([
            'abilities',
            'formula.main_ingredients',
            'formula.supplementary_ingredients',
            'path',
        ])
        .exec();
    // Deep copy main and supplementary ingredients
    const ingredients = [
        ...JSON.parse(JSON.stringify(sequence.formula.main_ingredients)),
        ...JSON.parse(
            JSON.stringify(sequence.formula.supplementary_ingredients)
        ),
    ];
...
const sequence_update_get = asyncHandler(async (req, res, next) => {
    ...
    ingredients[1] = ingredients[1].map((ingredient) => {
        if (
            sequence.formula.supplementary_ingredients.includes(ingredient._id)
        ) {
            return { ...ingredient._doc, checked: true };
        }
...
const sequence_create_post = [
    ...
    (req, res, next) => {
        ...
        if (!Array.isArray(req.body.main_ingredient)) {
            req.body.main_ingredient =
                typeof req.body.main_ingredient === 'undefined'
                    ? []
                    : [req.body.main_ingredient];
        }
        if (!Array.isArray(req.body.supplementary_ingredient)) {
            req.body.supplementary_ingredient =
                typeof req.body.supplementary_ingredient === 'undefined'
                    ? []
                    : [req.body.supplementary_ingredient];
        }
    ...
    body('main_ingredient')
        ...
        .escape(),
    body('main_ingredient.*').escape(),
    body('supplementary_ingredient')
        .custom((_, { req }) =>
            GreaterThanOrEquals('supplementary_ingredient', req)
        )
        .withMessage('Supplementary Ingredient must not be empty.')
        .escape(),
    body('supplementary_ingredient.*').escape(),
    body('ability')
    ...
    asyncHandler(async (req, res, _) => {
        ...
        if (!errors.isEmpty()) {
            ...
            ingredients[0] = ingredients[0].map((ingredient) => {
                if (
                    sequence.formula.main_ingredients.includes(ingredient._id)
                ) {
                    return { ...ingredient._doc, checked: true };
                }
            ...
            ingredients[1] = ingredients[1].map((ingredient) => {
                if (
                    sequence.formula.supplementary_ingredients.includes(
                        ingredient._id
                    )
                ) {
                    return { ...ingredient._doc, checked: true };
        ...
        const abilityExist = await Sequence.findOne({ name })
    ...
    asyncHandler(async (req, res, _) => {
        ...
        if (ThereIsImageAndUrl(imageBinary, oldSequence)) {
            await Cloudinary.update(
                dataURI,
                oldSequence.image.cloudinary_id,
                true
            );
        }
